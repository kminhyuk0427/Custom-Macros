# GTA 매크로


##  주요 기능(세부 설명은 config.py)

-  **mode설정**: 0=비활성 1=연속동작작 2=단일동작
-  **holds설정**: 특정 키를 몇 초 동안 누르고 있도록 설정 가능
-  **토글 키 지정**: 기본은 백틱키("`" 또는 "~")로 되어있음

##  주의사항

-  **중복 X**: 매크로 진행중에는 다른 매크로가 눌리지 않음
-  **반복 중단**: 매크로 반복동작 중에 키를 때면 그 즉시 동작중이던 매크로가 중지됨
-  **반복 동작 오류**: mode1(반복동작)에서 키를 매우 빠르게 눌렀다 때면 keyUP이 인식되지 않아 계속해서 입력될 수 있음
-  **씹덕 주의**: 아이콘 귀여움

##  실행법
### 1. 패키지 설치
```bash
pip install -r requirements.txt
```

### 2. 실행 파일 생성

-  다운로드 받은 폴더에서 build.bat를 실행하여 빌드

### 3. 실행 파일 생성

-  바탕화면에 이쁜 아이콘 클릭 후 실행(관리자 권한 자동으로 들어감)

##  매크로 설정

### 기본 예시
```python
MACROS = {
    '2': {
        'keys': ['3', '6'],
        'delays': [0.01, 0.01],
        'mode': 2
    },
}
```

### 홀드키 예
```python
't': {
    'keys': ['s', 'd'],
    'holds': [2.0, 0.01],  # s를 2초 동안 누름
    'delays': [0.1, 0.01],
    'mode': 2
}
```

### 반복 동작 예
```python
'3': {
    'keys': ['q', 'w', 'e'],
    'mode': 1 
}
```

##  종료하는 법
-  작업표시줄 숨김 아이콘에 귀여운 아이콘 있을꺼임, 우클릭 -> 종료

'
'
'
'
'
개인 커스텀 자유

참고 프로젝트: [Lester-Ver2.0]https://github.com/JU5TDIE/Lester-Ver2.0




ver - 1.4

수정사항
1. mod 0,1,2로 수정 (사용안함, 반복동작, 단일동작)
2. 매크로 동작 예외처리
3. 키홀드 기능 추가
4. 매크로 토글키 "~키"
5. README가이드 업데이트
6. 인게임 정상 동작 확인

(반복동작 모드시 os키입력이 섞이는 문제 있음)