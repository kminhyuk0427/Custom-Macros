import sys
import os
import threading
from pystray import Icon, Menu, MenuItem
from PIL import Image, ImageDraw

class TrayIcon:
    """시스템 트레이 아이콘"""
    __slots__ = ('on_exit_callback', 'icon', '_image')
    
    def __init__(self, on_exit_callback):
        self.on_exit_callback = on_exit_callback
        self.icon = None
        self._image = None
    
    def _create_default_icon(self):
        """기본 아이콘 생성"""
        if self._image:
            return self._image
        
        img = Image.new('RGB', (64, 64), 'black')
        d = ImageDraw.Draw(img)
        d.ellipse([8, 8, 56, 56], fill='green', outline='white', width=3)
        d.text((20, 15), 'M', fill='white')
        self._image = img
        return img
    
    def load_icon_image(self):
        """아이콘 로드"""
        if self._image:
            return self._image
        
        # 기본 경로 설정
        base = sys._MEIPASS if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
        icon_path = os.path.join(base, 'icon.ico')
        
        # 아이콘 로드 시도
        try:
            if os.path.exists(icon_path):
                self._image = Image.open(icon_path).resize((64, 64), Image.Resampling.LANCZOS)
                return self._image
        except:
            pass
        
        # 상위 폴더 시도
        parent_icon = os.path.join(os.path.dirname(base), 'icon.ico')
        try:
            if os.path.exists(parent_icon):
                self._image = Image.open(parent_icon).resize((64, 64), Image.Resampling.LANCZOS)
                return self._image
        except:
            pass

        # 기본 아이콘
        return self._create_default_icon()

    def on_quit(self, icon, item):
        """종료 처리"""
        icon.stop()
        if self.on_exit_callback:
            self.on_exit_callback()
        os._exit(0)

    def run(self):
        """트레이 아이콘 실행"""
        menu = Menu(
            MenuItem('KeyM', lambda: None, enabled=False), 
            MenuItem('종료', self.on_quit)
        )
        self.icon = Icon("KeyM", self.load_icon_image(), "KeyM", menu)
        threading.Thread(target=self.icon.run, daemon=True).start()